...
<script>
  export default {
    ...
    methods: {
      fetchResource () {
        this.$http.get('https://node-mu-nine.vercel.app/api/programmersresource/' + this.id).then(response => {
      ...
      deleteResource () {
        ...
        if (confirmDelete) {
          this.$http.delete('https://node-mu-nine.vercel.app/api/programmersresource/' + this.id).then(response => {
